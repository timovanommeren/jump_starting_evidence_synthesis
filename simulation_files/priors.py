...
def sample_priors(dataset: pd.DataFrame, seed: int) -> List[int]:
    ...
    if len(indices_w0) == 0 or len(indices_w1) == 0:
        raise ValueError("Need at least one row with label_included==0 and one with ==1 to sample minimal priors.")

    # Create a local random number generator with the seed (doesn't affect global state)
    rng = np.random.default_rng(seed)

    return [int(rng.choice(indices_w0)),
    int(rng.choice(indices_w1))]